...
class DeepseekV4ForConditionalGeneration(
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        # NOTE(dsvision): upstream does
        #     mapped = sorted(self.hf_to_vllm_mapper.apply(weights), ...)
        # which materializes EVERY checkpoint tensor at once. With this image's
        # InstantTensor (BUFFERED) loader those are real buffers, so an 88 GiB
        # checkpoint needs ~2x that and the box dies right after loading.
        # Stream the language weights straight through (they are already one
        # contiguous group, so the child's post-load finalization still sees a
        # fully loaded model) and defer only the small vision/aligner/image
        # tensors (~0.9 GiB) to a second pass.
        ...
        if deferred:
            loaded_params |= loader.load_weights(
                iter(sorted(deferred, key=lambda x: x[0]))
            )
