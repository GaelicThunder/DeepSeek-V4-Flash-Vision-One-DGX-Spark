...
try:  # vLLM >= the PR that added the named wrapper
    from vllm.multimodal.processing.processor import (  # type: ignore
        _plan_prompt_updates,
    )
except ImportError:  # older vendored vLLM: use the backported planner
    from .dsv4_mm_compat import _plan_prompt_updates  # type: ignore
...
class DeepseekV4VLMultiModalProcessor(
    ...
    def _call_hf_processor(
        ...
        Declaring this override is what matters: this vendored vllm routes
        image-only processing through `call_hf_processor_mm_only`, which
        assumes a real HF ProcessorMixin (it calls `processor._merge_kwargs`
        and then `processor.image_processor(images=[...])`, bypassing our
        `__call__` and its block building). The base class skips that path
        whenever `_call_hf_processor` is overridden, so images go through the
        text+mm path and reach our processor.
        """
        out = super()._call_hf_processor(prompt, mm_data, mm_kwargs, tok_kwargs)
        if "input_ids" not in out:
            # Our processor only produces image fields; this vllm's text+mm
            # path pops `input_ids` from the same BatchFeature, so tokenize
            # the prompt here (one nested list: it unpacks a single row).
