...
class Exl3MoEMethod(FusedMoEMethodBase):
    ...
    def _prepare_mixed_rank_sliced_weights(self, layer: RoutedExperts) -> None:
        ...
        unsupported = sorted(set(tiers).difference((2, 3, 4, 5, 6)))  # K2 enabled (dsvision)
        if unsupported:
            raise NotImplementedError(
    ...
    def _rank_sliced_runtime(
        ...
        # dsvision: with a mixed-K pack (K2 + a few K3 layers) the weight plan
        # differs per layer, and bind() rejects a scratch arena sized for another
        # plan ("experts do not match the plan used to size TP MoE scratch").
        # Key the runtime on the layer's weight plan so each K gets its own arena.
