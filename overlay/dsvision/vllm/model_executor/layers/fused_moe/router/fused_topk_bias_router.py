...
def _native_topk_softplus_sqrt_supports(num_experts: int) -> bool:
    ...
    return (
        num_experts in _NATIVE_TOPK_SOFTPLUS_SQRT_EXPERT_COUNTS
        and hasattr(torch.ops._moe_C, "topk_softplus_sqrt")
    )
...
def _reroute_image_tokens_(
    ...
    """Re-select experts for image tokens using bias_vl (DeepSeek V4 vision).

    Image tokens carry five consecutive in-vocab sentinel ids starting at
    image_sentinel_lo and pick experts with bias_vl instead of the regular
    correction bias or the hash table (vLLM PR #54566).

    CUDA-graph safe: no host synchronisation and no data-dependent shapes, so
    it must run for every row and blend by mask (`.any()`/`.nonzero()` would
    abort graph capture with "operation not permitted when stream is
    capturing"). The extra work is one softplus+topk over [tokens, experts].
    """
    ids = input_tokens.reshape(-1).long()
    mask = ((ids >= image_sentinel_lo) & (ids < image_sentinel_lo + IMAGE_SENTINEL_SPAN))
    mask = mask.unsqueeze(1)
    scores = torch.sqrt(F.softplus(gating_output.float()))
    choice = scores + bias_vl.reshape(1, -1).float()
    topk = topk_weights.shape[-1]
    _, sel = torch.topk(choice, k=topk, dim=-1)
...
def fused_topk_bias(
    ...
    vl_rerouting = bias_vl is not None and image_sentinel_lo > 0
    if vl_rerouting:
        assert input_tokens is not None, "bias_vl routing requires input_tokens"
        vl_input_tokens = input_tokens
    if (
...
class FusedTopKBiasRouter(BaseRouter):
    ...
    def __init__(
        ...
        *,
        ...
        self._bias_vl = bias_vl
        self._image_sentinel_lo = int(image_sentinel_lo)
    ...
    def _compute_routing(
        ...
        *,
        ...
        topk_weights, topk_ids = fused_topk_bias(
            ...
            bias_vl=(
                self._bias_vl.data if self._bias_vl is not None else None
            ),
            image_sentinel_lo=self._image_sentinel_lo,
            routed_scaling_factor=self.routed_scaling_factor,
        )
