...
| **Text-only view does not boot with the current overlay** (`KeyError: 'model.layers.0.ffn.gate.bias_vl'`) | the router of the vision port expects the 46 `gate.bias_vl` tensors, which the text index does not list; serve the vision view (`tools/use_vision.sh on`). The `.text`/`.vision` manifests of a spliced pack must also be regenerated with the promoted file sizes (`scripts/kalibrated/make_aplus_pack.sh` now does; the published pack was fixed on 2026-09-07) |
